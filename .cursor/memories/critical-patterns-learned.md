...
30. ❌ **Using underscore prefix for public properties in ViewModels** → ✅ Public properties (including ViewModel fields) should use CamelCase, not underscore prefix
31. ❌ **Mixing naming patterns within same scope** → ✅ Be consistent: Private use `_snakeCase`, Public use `CamelCase`, Local use `l{typePrefix}`
32. ❌ **Not updating all references when renaming variables** → ✅ Always search and replace systematically to update all usages
33. ❌ **Assuming all ViewModel fields are private** → ✅ Check access modifier - public fields for data binding should use CamelCase
...
### 1. [Inject] Properties
- **Always private** and use **CamelCase**
...
### 8. Complete Examples
```csharp
// [Inject] properties - private, CamelCase, assign default!
...
- **Public**: `public Type PropertyName { get; set; }` (CamelCase, NO type prefix)
...
2. **Apply correct pattern**: 
   ...
   - Public: `CamelCase` (no type prefix)
...
- [ ] Variable naming follows correct patterns: Private `_snakeCase`, Public `CamelCase`, Local `l{typePrefix}`
- [ ] Public properties in ViewModels use CamelCase (not underscore prefix)
